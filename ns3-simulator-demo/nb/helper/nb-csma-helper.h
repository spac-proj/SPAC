 ...
 */
#ifndef NB_CSMA_HELPER_H
#define NB_CSMA_HELPER_H
...
/**
 * @ingroup csma
 * @brief build a set of NbCsmaNetDevice objects
 *
 ...
 */
class NbCsmaHelper : public PcapHelperForDevice, public AsciiTraceHelperForDevice
{
  public:
    /**
     * Construct a NbCsmaHelper.
     */
    NbCsmaHelper();

    ~NbCsmaHelper() override
    {
    }
     ...
     * NbCsmaNetDevice created through NbCsmaHelper::Install.
     */
     ...
     *
     * Set these attributes on each ns3::NbCsmaNetDevice created
     * by NbCsmaHelper::Install
     */
     ...
     * @param v1 the value of the attribute to set
     *
     * Set these attributes on each ns3::NbCsmaChannel created
     * by NbCsmaHelper::Install
     */
    ...
    /**
     * This method creates an ns3::NbCsmaChannel with the attributes configured by
     * NbCsmaHelper::SetChannelAttribute, an ns3::NbCsmaNetDevice with the attributes
     * configured by NbCsmaHelper::SetDeviceAttribute and then adds the device
    ...
    /**
     * This method creates an ns3::NbCsmaNetDevice with the attributes configured by
     * NbCsmaHelper::SetDeviceAttribute and then adds the device to the node and
     ...
     * @returns A container holding the added net device.
     */
    NetDeviceContainer Install(Ptr<Node> node, Ptr<NbCsmaChannel> channel) const;
     ...
     * @returns A container holding the added net device.
     */
    NetDeviceContainer Install(std::string nodeName, Ptr<NbCsmaChannel> channel) const;
    ...
    /**
     * This method creates an ns3::NbCsmaChannel with the attributes configured by
     * NbCsmaHelper::SetChannelAttribute.  For each Ptr<node> in the provided
     * container: it creates an ns3::NbCsmaNetDevice (with the attributes
     * configured by NbCsmaHelper::SetDeviceAttribute); adds the device to the
     ...
     * ns3::NbCsmaNetDevice (with the attributes configured by
     * NbCsmaHelper::SetDeviceAttribute); adds the device to the node; and attaches
     ...
     * @returns A container holding the added net devices.
     */
    NetDeviceContainer Install(const NodeContainer& c, Ptr<NbCsmaChannel> channel) const;
     ...
     *          NbCsmaNetDevice should be modified to use a fixed stream
  ...
  private:
     ...
     * @returns A container holding the added net device.
     */
    Ptr<NetDevice> InstallPriv(Ptr<Node> node, Ptr<NbCsmaChannel> channel) const;
...
template <typename... Ts>
void
NbCsmaHelper::SetQueue(std::string type, Ts&&... args)
{
