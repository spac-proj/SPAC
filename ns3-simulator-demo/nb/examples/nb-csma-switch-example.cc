...
#include "ns3/nb-csma-switch-port-device.h"
#include "ns3/nb-csma-switch-stack.h"
...
NS_LOG_COMPONENT_DEFINE("NbSwitchSimple");
...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("NbCsmaSwitchStack",   LOG_LEVEL_INFO);
    ...
    /* ---------- create switch stack ---------- */
    Ptr<NbCsmaSwitchStack> sw = CreateObject<NbCsmaSwitchStack>();

    /* ---------- common CSMA parameters ---------- */
    NbCsmaHelper csma;
    csma.SetChannelAttribute("DataRate", DataRateValue(1000000)); // 1 Mbps for slower simulation
    csma.SetChannelAttribute("Delay",    TimeValue(MilliSeconds(5)));
    printf("CSMA channel (Backoff Enabled): DataRate 1Mbps, Delay 5ms\n");
    ...
    {
        // 1. independent point-to-point CSMA link
        Ptr<NbCsmaChannel> ch = CreateObject<NbCsmaChannel>();
        ch->SetAttribute("DataRate", DataRateValue(1000000));
        ch->SetAttribute("Delay",    TimeValue(MilliSeconds(5)));

        // 2. host side normal NbCsmaNetDevice
        NetDeviceContainer tmp = csma.Install(hosts.Get(i), ch);
        hostDevs[i] = tmp.Get(0);

        // 3. switch side NbCsmaSwitchPortDevice
        Ptr<NbCsmaSwitchPortDevice> port = CreateObject<NbCsmaSwitchPortDevice>();
        port->SetNode(swNode);
        swNode->AddDevice(port);
        port->Attach(ch);           // physical connection
        sw->AttachPort(port);       // attach to switch stack
    }
