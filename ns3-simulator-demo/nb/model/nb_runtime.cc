...
struct data_queue_t* nb__new_data_queue(void) {
	struct data_queue_t * q = (struct data_queue_t *)malloc(sizeof(struct data_queue_t));
	q->current_elems = 0;
	return q;
}
...
nb__accept_queue_t* nb__new_accept_queue(void) {
	nb__accept_queue_t* q = (struct accept_queue_t *)malloc(sizeof(*q));
	q->current_elems = 0;
	return q;
}
...
void nb__cycle_connections(void) {
	int i;
...
unsigned int nb__my_host_id = 0;
unsigned long long nb__my_local_host_id = 0;

nb__net_state_t* nb__net_state = NULL;
